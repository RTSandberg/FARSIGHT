...
int main(int argc, char** argv) {
    ...
    json deck;
    try {
        json_stream = std::ifstream(input_deck);
        json_stream >> deck;
    ...
    double x_min = deck.at("xmin"), x_max = deck.at("xmax"), v_min = deck.at("vmin"), v_max = deck.at("vmax");
    ...
    double kx = 2.0 * M_PI / Lx * deck["normalized_wavenumber"].get<int>();
    double amp = get_json_val<double>(deck, "amp", 0.0);//0.5;
    double vth = get_json_val<double>(deck, "vth", 1.0);//atof(argv[9]);//1.0;
    double vstr = get_json_val<double>(deck, "vstr", 0.0); //atof(argv[10]);
    int sim_type = get_json_val<int>(deck, "sim_type", 1);//atoi(argv[6]);
    ...
    int initial_height = deck.at("initial_height");//atoi(argv[11]);//6; 
    int max_height = get_json_val<int>(deck, "max_height", initial_height);
    double greens_epsilon = deck.at("greens_epsilon");//atof(argv[12]);//0.2;
    int use_treecode = get_json_val<int>(deck, "use_treecode", 0); //atoi(argv[13]);
    double beta = get_json_val<double>(deck, "beta", -1.0); //atof(argv[14]);
    double mac = get_json_val<double>(deck, "mac", -1.0); //atof(argv[15]);
