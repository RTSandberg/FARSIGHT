...
// F0_M functions
F0_M::F0_M() : vth(1.0), vstr(0.0) {}
F0_M::F0_M(double vth) : vth(vth), vstr(0.0) {}
F0_M::F0_M(double vth, double vstr) : vth(vth), vstr(vstr) {}

double F0_M::operator() (double x, double v) {
    return 1.0 / sqrt(2.0 * M_PI) / vth * exp(-(v-vstr)*(v-vstr) / 2 / vth /vth);
}
double F0_M::get_vth() { return vth; }
void F0_M::print() {
    cout << "Maxwellian ics" << endl;
    cout << "vth = " << vth << "vstr = " << vstr << endl;
}
// end F0_M functions

// F0_LD functions
F0_LD::F0_LD() : vth(1.0), vstr(0.0), k(.5), amp(.1) {}
F0_LD::F0_LD(double vth, double k, double amp) 
    : vth(vth), vstr(0.0), k(k), amp(amp) {}
F0_LD::F0_LD(double vth, double vstr, double k, double amp) 
    : vth(vth), vstr(vstr), k(k), amp(amp) {}
...
void F0_LD::print() {
    ...
    cout << "vth = " << vth << "vstr = " << vstr << ", amp = " << amp << ", k = " << k << endl;
}
// end F0_LD functions
